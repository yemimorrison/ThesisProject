...
prepare_data_list <- function(dat,
  ...
  dat <- dat %>% mutate(delay = rep_date - event_date)

  for (t in 1:cap_T) {
    data.att <- dat[which(dat$event_date == t02s[t]), ]
    for (d in 0:(cap_T - t)) {
...
evaluate_nowcast <- function(dat, model, now, D_max = 35) {
  ...
  now_date <- format(now, "%b %d %Y")
  FHM_ICU <- read_excel(paste0("./data/mytests/Folkhalsomyndigheten_Covid19_", now_date, ".xlsx"),
    sheet = "Antal intensivvårdade per dag"
  )

  FHM_cases <- read_excel(paste0("./data/mytests/Folkhalsomyndigheten_Covid19_", now_date, ".xlsx"),
  ...
  ts <- FHM_ICU %>%
    ...
    left_join(FHM_cases %>% select(date = Statistikdatum, n_cases = Totalt_antal_fall), by = "date") %>%
    mutate(
  ...
  if (model == "mod_l") {
    ...
    samples <- sampling(mod, data = list(
      ...
      Z = prep_dat_list$Z,
      alpha = rep(1, prep_dat_list$maxDelay + 1)
    ), seed = 1142, chains = 4, iter = 2000, control = list(adapt_delta = 0.98))
  ...
  for (j in 1:length(save_res)) {
    ...
    write_csv(draws_long, paste0("./results/", save_res[j], "/", save_res[j], "_", model, "_", now, ".csv"))
  }
}
...
#now <-"2020-11-24"
#now <- as.Date(now)
#model <- stan_model("./code/stan_models/mod_l.stan")
#model <- "mod_r"
#evaluate_nowcast(covid_death_df,model,now = now,D_max = 35)

#covid_death_df <- covid_death_df %>% filter(event_date > 0)
